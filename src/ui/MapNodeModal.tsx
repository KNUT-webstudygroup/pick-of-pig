...
import { placeIdToPhoneNumber } from '@/service/search';
...
function MapNodeModal({ index, node }: MapNodeCardProps) {
  ...
  useEffect(() => {
    ...
    // const phoneNumber = placeIdToPhoneNumber(node.id, map);
  }, [node]);
...
const MapNodeModalContainer = styled.div`
    ...
    align-items: center;
    background-color: rgba(255, 154, 159, 0.8);
...
const MapNodeModalSubTitle = styled.div`
    display: flex;
    flex-direction: row;
